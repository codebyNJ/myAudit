...
func parseEnvelope(b []byte) agent.Result {
	var e envelope
	if err := json.Unmarshal(b, &e); err != nil {
		return agent.Result{Err: "parse envelope: " + err.Error()}
	}
	r := agent.Result{
		Summary: e.Result,
		CostUSD: e.CostUSD,
...
func Run(ctx context.Context, ws sandbox.Workspace, task string, opt Options) (agent.Result, error) {
	...
	if err != nil {
		...
		return agent.Result{}, fmt.Errorf("claude exec: %s", msg)
	}
	return agent.Result{}, fmt.Errorf("claude produced no output")
}

func runStreaming(cmd *exec.Cmd, onStep func(string)) (agent.Result, error) {
	...
	if !haveFinal {
		...
		return agent.Result{}, fmt.Errorf("claude stream: %s", msg)
	}
	return final, nil
}
